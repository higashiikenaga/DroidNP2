...
| `insert_disk` | FD1/FD2へディスクを挿入します。`url`(CORS対応のfetch元。Dropboxの公開共有リンクはホスト名置換で直接取得、Google Driveの公開共有リンクは中継経由で取得可、OneDriveは不可)・`source_key`(ライブラリから)・`blank`(FAT12フォーマット済みの空FD)のいずれか1つを指定します。 |
...
| `disk_list_files` | FD1/FD2にマウント中のディスクイメージ(FAT12/16)内のファイル一覧と空き容量を、ゲストOSを介さず直接読み取ります。HDDスロットは対象外(コアが実行中のHDD挿抜に未対応のため)。起動前のHDDイメージはUIのファイル転送ダイアログから編集できますが、この経路では扱えません。ファイル名は8.3形式のみ。 |
...
| `put_file` | ホストのテキスト/バイナリを転送用FD経由でゲストの任意ドライブ(HDD含む)へ配置します。転送用FDへ書いてからゲストDOSに`COPY`させる複合ツールで、**HDDイメージをホストが直接書き換えないためDOS側キャッシュと衝突せず安全です。実行中のHDDへの書き込みはこの経路を使うこと**(起動前であればこの制約はなく、セットしただけのHDDはUIのファイル転送ダイアログから直接編集できます。そちらはUI専用でMCPからは扱えません)。 `path`はゲスト側の宛先フルパス(例`A:\WORK\FOO.TXT`)、ファイル名は8.3形式のみ。DOSプロンプト待ち状態で実行すること。 |
| `get_file` | ゲストの任意ドライブ(HDD含む)上のファイルを転送用FD経由でホストへ取り出します。ゲストDOSに`COPY`させてからホストがFDを直接読む複合ツールで、**実行中のHDDからの読み出しもこの経路を使うのが安全です**(起動前のHDDはUIのファイル転送ダイアログから直接読めます)。 DOSプロンプト待ち状態で実行すること。 |
| `read_memory` | デバッグ/解析用。PC-98メインRAM(0x00000-0x1FFFFF)の任意範囲をゲストOSを介さず直接base64で読み出します。`addr`+`len`はRAMサイズ以内、`len`は1MB(1048576バイト)以内に制限されます。通常のファイル取得は`get_file`/`put_file`を使うこと。 |
