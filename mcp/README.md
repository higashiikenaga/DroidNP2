...
| `insert_disk` | FD1/FD2へディスクを挿入します。`url`(CORS対応のfetch元。Google Drive/Dropboxは公開共有リンクなら中継経由でも取得可、OneDriveは不可)・`source_key`(ライブラリから)・`blank`(FAT12フォーマット済みの空FD)のいずれか1つを指定します。 |
