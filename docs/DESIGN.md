...
- ツールバー常設: リセット / フルスクリーン / ソフトキーボード（表示中は直後に⌨/🎮切替）/
  スクリーンショット / 「…」。低頻度操作は「…」内の入力・ディスク・ステート3グループと
  ROM登録・デバッガ・使い方・言語の直結行へ整理する
...
切り替える。同時にFDスロット行を隠すのは、パネル分の高さを確保しようとして`rescale()`がさらに
canvasを縮め、それに応じてパネルの必要高さも変わり…という収縮ループを避けるため（FDスロット行は
操作頻度が低く、隠しても再表示は入力パネルを閉じるだけで済む）。`kbdPanel`/`trackpadPanel`の
