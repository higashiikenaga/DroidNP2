...
- ソフトキーボードにはPC-98のテンキーもあり、物理テンキーのないノートPCでも
  テンキー移動専用ソフトを操作できます。
...
- ソフトキーボードかバーチャルパッドの表示中は、キーボードボタン直後に **⌨／🎮** の
  切替チップが現れます。パッド表示中に🎮をもう一度押すとプロファイル選択・割当編集ができます。

常時表示するツールバーボタンは、リセット／フルスクリーン／ソフトキーボード／
スクリーンショット／「…」です。「…」内は入力／ディスク／ステートの3グループと、
ROM登録／デバッガ／使い方／言語の直結行で構成されます。言語行は地球儀と現在値を表示します。
...
- スマホ対応（タッチ操作・テンキー付きPC-98配列ソフトキーボード・自動配置バーチャルパッド）
