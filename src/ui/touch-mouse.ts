/**
 * タッチマウス(タッチ操作をPC-98のマウス入力へ変換する)のジェスチャ認識。
 *
 * iOS Safari は Pointer Lock API に対応しておらず、キャプチャモードが成立しない。かといって
 * タッチ位置をそのままカーソルの目標位置にする方式(絶対位置追従)は、カーソルが常に指の
 * 真下に来て指の影に隠れて見えないという実使用上の難点があり、既存の canvas 直タッチ処理
 * (player.ts の絶対追従)とは別に、トラックパッド式(相対移動)を追加する。このモジュールは
 * その「タッチ列 → マウス操作」の解釈だけを受け持つ。
 *
 * DOM/BOM には一切触れない純ロジックとして呼び出し側(virtual-trackpad.ts)から分離した
 * (WebNP2/WebX68k 共通の切り出し方針。タッチ実機なしで vitest からジェスチャ判定を検証するため)。
 ...
 * バスマウスも左右2ボタンのみでホイールという概念自体が無く、NP2kai側(コアのマウス処理)も
 * 左右しか読んでいないため、実装しても受け取り先が無い(WebX68k版と同じ理由)。
 */
