 ...
 *
 * canvas を直接タッチの受け口にする既存処理(player.ts のcanvasタッチイベント。絶対位置追従)は
 * 「ゲーム画面そのもの」であり、操作面としては指の影に隠れる・誤タップでゲスト側の描画や
 * クリックを誘発するといった難点がある。そこで仮想キーボード/バーチャルパッドと同じ
 * 「入力パネル」の1つとして、専用の矩形領域(このファイルが受け持つ)を用意し、そこで
 * 指の動きを解釈する(既存の canvas 直タッチ処理はそのまま残し、この機能はあくまで
 * 入力パネルの選択肢を1つ増やすだけ)。
 *
 ...
 * パネルの表示/非表示の管理だけ。CSSピクセル→ゲストのドット数への換算はホスト固有の
 * マウス管理の知識(main.ts の領分)なので、呼び出し側のコールバックに委ねる。
 *
 * 2本指ドラッグ(トラックパッドの慣例でホイール相当に使われる操作)は実装しない。PC-98の
 * バスマウスも左右2ボタンのみでホイールという概念自体が無く、NP2kai側も左右しか読んでいない
 * ため、実装しても受け取り先が無い。
 */

import { TouchMouse, type TouchMouseButton } from './touch-mouse.ts';
...
export interface VirtualTrackpadCallbacks {
   ...
   * 呼び出し側が持つ「送信残差」のようなストローク単位の状態を次のストロークへ
   * 持ち越さないために使う(TouchMouse 自身はそうした状態を知らないため、ここで通知する)。
   */
  strokeEnd(): void;
}
...
export function createVirtualTrackpad(el: HTMLElement, callbacks: VirtualTrackpadCallbacks): VirtualTrackpad {
  ...
  el.addEventListener('pointerdown', (e) => {
    ...
    // 捕捉できなくても要素外へ出た瞬間の追従が切れるだけなので握りつぶす。
    try {
